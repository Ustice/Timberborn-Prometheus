...
| 2026-04-27 | P2S-025 worker and orchestrator checks: `bash scripts/test.sh` + `bash scripts/build.sh --qa` + copied fixture load attempt | Blocked | Farmhouse amendment prototype is committed on `codex/P2S-025-implement-farmhouse-amendment` at `e2c70ac`; worktree tests passed 90 tests and `--qa` built/deployed. The copied `Prometheus P2S-025 QA` fixture has farmhouse, carrots, and injected District Center `FertileAsh`, but loading it through the known mod-version mismatch warning crashed during `Timberborn.DwellingSystem.Dweller.Load`. Error report: `/Users/jasonkleinberg/Documents/Timberborn/Error reports/error-report-2026-04-27-04h47m00s.zip`. |
...
| Fertile Ash farmhouse application live QA | Blocked | Provide or create a loadable fixture with a finished farmhouse, eligible crop planting spots, and stored `FertileAsh`. The copied `Prometheus P2S-025 QA` fixture crashes during `Dweller.Load`; do not integrate P2S-025 until `fertile_ash_farmhouse_amendment_applied`, decreased stored ash, and faster amended crop growth than a nearby control crop are captured. |
...
Resolve P2S-025's live QA gate with a loadable farmhouse/crop/ash fixture. The current copied fixture crashes during `Dweller.Load`, so the fastest path is either a fresh in-game fixture or a repaired save that loads normally. Then capture ash consumption and amended-vs-control crop growth evidence from `Fire.log`, `Player.log`, and the game UI. Do not start P2S-027 until P2S-025 is either integrated or explicitly descoped.
