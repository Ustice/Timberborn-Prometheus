...
| 2026-04-27 | Fertile Ash farmhouse amendment | Recovery | Blocked | `bash scripts/test.sh`, `bash scripts/build.sh --qa`, copied save fixture, `Player.log`, `Fire.log` | Prototype branch `codex/P2S-025-implement-farmhouse-amendment` at `e2c70ac` passed 90 tests and built/deployed. The copied `Prometheus P2S-025 QA` fixture crashed during `Timberborn.DwellingSystem.Dweller.Load`; `Fire.log` did confirm the recovery adapter resolved `PlantingSpotFinder.FindClosest(Vector3)` before the crash. Required evidence remains ash consumption, `fertile_ash_farmhouse_amendment_applied`, and faster amended crop growth than a nearby control. |
