...
namespace Mods.Prometheus.Scripts {
  ...
  internal static class FireNativeParticleSourceCatalog {
    ...
    public static GameObject TryGetRecommendedSource(FireVisualEffectKind kind) {
      foreach (var sourceName in GetRecommendedSources(kind)) {
        var source = TryGetSource(sourceName);
        if (source != null) {
          return source;
        }
      }
    ...
    private static void EnsureSearched() {
      ...
      var resourceParticlePrefabs = Resources.LoadAll<GameObject>(string.Empty)
        .Where(gameObject => gameObject != null && gameObject.GetComponentsInChildren<ParticleSystem>(true).Length > 0);
      foreach (var prefab in resourceParticlePrefabs) {
        if (!SourcesByName.ContainsKey(prefab.name)) {
          SourcesByName[prefab.name] = prefab;
        }
      }
    }
